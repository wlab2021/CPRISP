...
def create_model(seq_len, vocab_size, n_annotations, d_hidden_seq = 128, d_hidden_global = 512, n_blocks = 6, n_heads = 4, \
    ...
    for block_index in range(1, n_blocks + 1):
        ...
        dense_global = keras.layers.Dense(d_hidden_global, activation = activation, name = 'global-dense2-block%d' % block_index)(hidden_global)  # 全连接层
        hidden_global = keras.layers.Add(name = 'global-merge2-block%d' % block_index)([hidden_global, dense_global])  # 用于实现跳跃连接
        hidden_global = keras.layers.LayerNormalization(name = 'global-merge2-norm-block%d' % block_index)(hidden_global)  # 用于实现归一化

    # 对应论文流程图的softmax、sigmoid操作
    ...
    # hidden_seq：local representations
    # hidden_global: global representations

    return keras.models.Model(inputs = [input_seq, input_annoatations], outputs=[output_seq, output_annotations, hidden_seq, hidden_global])
...
def get_model_with_hidden_layers_as_outputs(model):
    ...
    return keras.models.Model(inputs=model.inputs, outputs=[model.outputs[2], model.outputs[3]])
