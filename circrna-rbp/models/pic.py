...
class PIC_no(nn.Module):
    def __init__(self, configs, device):
        ...
        self.projection_head_pic = nn.Sequential(
            ...
            # nn.Linear(157, 100),  # 5 types of C ircRNA2Vec+pseudo-peptide+K nucleotide frequency (KNF)+EIIP-ANF-CCN +structural information and sequence information
            # nn.Linear(162, 100),

            # nn.Linear(138, 100),  # Pseudoamino acids + K nucleotide frequency (KNF) + EIIP-ANF-CCN + structural information and sequence information
            # nn.Linear(143, 100),

            # nn.Linear(127, 100),  # Pseudo-peptide + K nucleotide frequency (KNF) + EIIP-ANF-CCN + structural information and sequence information
            # nn.Linear(132, 100),

            nn.Linear(147, 100),  # CircRNA2Vec + K nucleotide frequency (KNF) + EIIP - ANF - CCN + structural information and sequence information
            # nn.Linear(152, 100),

            # nn.Linear(84, 100),  # K nucleotide frequency (KNF)
            # nn.Linear(89, 100),

            # nn.Linear(9, 100),  # EIIP - ANF - CCN
            # nn.Linear(14, 100),

            # nn.Linear(24, 100),  # Structural information and sequence information
            # nn.Linear(5, 100),
    ...
    # def forward(self, features_aug1,RBP_aug, RBP_aug1, RBP_aug2, output,labels):
    def forward(self, output): #, features_aug4, features5, features6
        c_t = output

        yt = self.projection_head_pic(c_t).squeeze(dim=1)  # no-RBP
