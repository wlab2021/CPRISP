...
coden_dict = {'AGO1': 0, 'AGO2': 1, 'AGO3': 2, 'ALKBH5': 3, 'AUF1': 4, 'C17ORF85': 5, 'C22ORF28': 6, 'CAPRIN1': 7, 'DGCR8': 8, 'EIF4A3': 9, 'EWSR1': 10,
               'FMRP': 11, 'FOX2': 12, 'FUS': 13, 'FXR1': 14, 'FXR2': 15, 'HNRNPC': 16, 'HUR': 17, 'IGF2BP1': 18, 'IGF2BP2': 19, 'IGF2BP3': 20,
               'LIN28A': 21, 'LIN28B': 22, 'METTL3': 23, 'MOV10': 24,  'PTB': 25, 'PUM2': 26, 'QKI': 27, 'SFRS1': 28, 'TAF15': 29, 'TDP43': 30,
...
def Trainer(data_type, epochs, model,hdrnet, temporal_contr_model, model_optimizer, temp_cont_optimizer, train_dl, test_dl, device,
    ...
    typeF = "Randomly generate RBP—structure_withRBP"
    for epoch in range(1, epochs + 1):
        # Train and validate
        train_loss, train_acc, train_auc, train_precision, train_recall = model_train(data_type,model, hdrnet,temporal_contr_model, model_optimizer, temp_cont_optimizer,
                                            criterion, train_dl, config, device, training_mode,typeF)
        test_loss, test_acc, test_auc, test_precision, test_recall, _, _ = model_evaluate(data_type,model,hdrnet, temporal_contr_model, test_dl, device, training_mode,typeF)
...
def model_train(data_type,model, hdrnet,temporal_contr_model, model_optimizer, temp_cont_optimizer, criterion, train_loader, config,
    ...
    for batch_idx, (data1, data2, data3,data4,data5,labels,idxs) in enumerate(train_loader):
        ...
        data1, data2, data3,data4,data5 = data1.float().to(device), data2.float().to(device), data3.float().to(device),data4.float().to(device),data5.float().to(device)
        ...
        RBP_37n = RBP_37.detach().numpy()


        features1 = model(data1,tag=1)  #KNF
        features2 = model(data2,tag=2)  #EIIP-ANF-CCN

        features3 = model(data3,tag=3)
        features4 = model(data4,tag=3)  #circRNA2Vec
        ...
        prob = prob1.detach().numpy()
        idxs1 = idxs.detach().numpy()
